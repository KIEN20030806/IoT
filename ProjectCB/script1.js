const firebaseConfig = {
  ...
  firebase.database().ref("/Bedroom/Tem").on("value",function(snapshot){
    var nd = snapshot.val();  
    ...
    $("#Hum").roundSlider({
      ...
      firebase.database().ref("/Bedroom/Hum").set(newValue); // Ghi giá trị mới vào cơ sở dữ liệu Firebase
    }
    });  
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      minusButton.addEventListener('click', function() {
          ...
          if (currentValue > 0) {
              ...
              firebase.database().ref("/Bedroom/Soluong").set(currentValue - 1);
          }
      });
    ...
    motionCheckbox.addEventListener("change", function() {
        if (this.checked) {
          ...
          firebase.database().ref("/Bedroom/Quat").set("ON");
        } else {
          document.getElementById("Quat").src = "./img/quatoff.png";
          firebase.database().ref("/Bedroom/Quat").set("OFF");
        }
    });
  ...
  document.getElementById("myRange").addEventListener("input", function() {
    ...
    firebase.database().ref("/Bedroom/Light").set(lightValue);
  ...
  function toggleLED() {
      ...
      if (ledIndicator.classList.contains("led-red")) {
          ...
          firebase.database().ref("/Bedroom/LED").set(0);
      } else {
          // Nếu không, chuyển về đỏ
          ledIndicator.classList.add("led-red");
          firebase.database().ref("/Bedroom/LED").set(1);
      }
  }
